...
namespace Trading
{
	class OrderGateway 
	{
		...
		private: 
			...
			/*Lock free queue on which we write client responses which we read and procesed from the exchange, to be consumed by the trade endgine.*/
		...
		private: 
			/*Main thread loop - sends out client requests to the exchange and reads and dispaches incoming client responses.*/
			auto run() noexcept -> void; 

			/*Callback when an incoming client response is read, we perform some checks and forward to to the lock free queue connected to the trade engine.*/
