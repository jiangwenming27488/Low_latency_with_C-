...
namespace Exchange
{
	...
	auto SnapshotSynthesizer::addToSnapshot(const MDPMarketUpdate *market_update)
	{
		...
		switch (me_market_update.type_)
		{
			...
			case MarketUpdateType::MODIFY:
				{
					...
					ASSERT(order->side_ == me_market_update.side_, "Expecting exising order to match new one."); 
			...
			case MarketUpdateType::CANCEL: 
				{
					auto order = orders->at(me_market_update.order_id_); 
					ASSERT(order != nullptr, "Received:" + me_market_update.toString() + " but order does not exst."); 
					ASSERT(order->order_id_ == me_market_update.order_id_, "Expecting existing order to match new one."); 
	...
	/*Main method for this thread - process incremental updates from the market data publisher, updates the snapshot and publishes the snapshot periodically*/
