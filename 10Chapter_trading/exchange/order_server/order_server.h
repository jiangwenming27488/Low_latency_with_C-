...
namespace Exchange
{
	class OrderServer
	{
		public: 
			...
			/*Main run loop for this thread - accepts new client connections, receives requests from them and sends client responses to them*/
			auto run() noexcept
			{
				...
				while (run_)
				{
					...
					for (auto client_response = outgoing_responses_->getNextToRead(); outgoing_responses_->size() && client_response; client_response = outgoing_responses_->getNextToRead())
					{
						...
						ASSERT(cid_tcp_socket_[client_response->client_id_] != nullptr, "Donot have a TCPSocket for ClientId:" + std::to_string(client_response->client_id_)); 
		...
		private: 
			...
			/*TCP server instance listening for new client connectins*/
			Common::TCPServer tcp_server_; 

			/*FIFO sequencer responsible for making sure incoming client requests are processed in the order in which they where received */
