...
namespace Trading
{
	MarketDataConsumer::MarketDataConsumer(Common::ClientId client_id, Exchange::MEMarketUpdateLFQueue *market_updates, const std::string &iface, const std::string &snapshot_ip, int snapshot_port, const std::string &incremental_ip, int incremental_port) : incoming_md_updates_(market_updates), run_(false), logger_("trading_market_data_consumer_" + std::to_string(client_id) + ".log"), incremental_mcast_socket_(logger_), snapshot_mcast_socket_(logger_), iface_(iface), snapshot_ip_(snapshot_ip), snapshot_port_(snapshot_port)
	{
		...
		ASSERT(incremental_mcast_socket_.join(incremental_ip), "Join faile on:" + std::to_string(incremental_mcast_socket_.socket_fd_) + " error:" + std::string(std::strerror(errno))); 

		snapshot_mcast_socket_.recv_callback_ = recv_callback; 
	}

	/*Main loop for this thread = reads and processes messages from the multicast socket - the heavy lifting is in the "recvCallback()" and "checkSnapshotSync()" methods*/
	...
	auto MarketDataConsumer::checkSnapshotSync() -> void 
	{
		...
		for (auto inc_itr = incremental_queued_msgs_.begin(); inc_itr != incremental_queued_msgs_.end(); ++inc_itr)
		{
			...
			if (inc_itr->first != next_exp_inc_seq_num_)
			{
				logger_.log("%:% %() % Detected gap incremental stream expected:% found:% %\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_), next_exp_inc_seq_num_, inc_itr->first, inc_itr->second.toString()); 
		...
		logger_.log("%:% %() % Recovered % snapshot and % incremental order.\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_), snapshot_queued_msgs_.size() - 2, num_incrementals); 

		snapshot_queued_msgs_.clear(); 
		incremental_queued_msgs_.clear(); 
		in_recovery_ = false; 

		snapshot_mcast_socket_.leave(snapshot_ip_, snapshot_port_);//';' /*was left in
	}
	...
	auto MarketDataConsumer::recvCallback(McastSocket *socket) noexcept -> void
	{
		...
		{/*market udpdate was read from the snapshot market data stream and we are not in recovery, so we dont need it and discard it.*/
		...
		if (socket->next_rcv_valid_index_ >= sizeof(Exchange::MDPMarketUpdate))
		{
			...
			for (; i + sizeof(Exchange::MDPMarketUpdate) <= socket->next_rcv_valid_index_; i += sizeof(Exchange::MDPMarketUpdate))
			{
				...
				if (UNLIKELY(in_recovery_))
				{
					...
					queueMessage(is_snapshot, request); /*queue up the market data update message and check if snapshot recovery / synchronization can be completed sucessfully*/
				} else if (!is_snapshot)
