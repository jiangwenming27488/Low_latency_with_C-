...
namespace Common
{
	...
	{
		/* Note that needs_so_timestamp=true fo rFIFOSequencer*/
	...
	auto TCPSocket::sendAndRecv() noexcept -> bool 
	{
		...
		if (read_size > 0)
		{
			...
			if (cmsg->cmsg_level == SOL_SOCKET && cmsg->cmsg_type == SCM_TIMESTAMP && cmsg->cmsg_len == CMSG_LEN(sizeof(time_kernel)))
			{
				...
				kernel_time = time_kernel.tv_sec * NANOS_TO_SECS + time_kernel.tv_usec * NANOS_TO_MICROS; /*convert timestamp to nanosecods*/
			}

			const auto user_time = getCurrentNanos(); 

			logger_.log("%:% %() % read socket:% len:% utime:% ktime:% deff:%\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_), socket_fd_, next_rcv_valid_index_, user_time, kernel_time, (user_time - kernel_time)); 
