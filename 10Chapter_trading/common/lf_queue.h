...
namespace Common
{
	...
	{
		public: 
			explicit LFQueue(std::size_t num_elems) : store_(num_elems, T()) /*pre-allocation of vector storge*/
			{
			...
			{
				return (size() ? &store_[next_read_index_] : nullptr); /*its comparing the nullptr "ponter" and "&"*/
			}
		...
		private: 
			...
			/*Atomic trackers for next index to write new data to and read data from*/
