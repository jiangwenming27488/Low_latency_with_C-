...
namespace Common
{
	struct TCPServer
	{
		...
		/*Publish outgoing data from the send buffer and read incoming from the receive buffer. */
		auto sendAndRecv() noexcept -> void; 

		private: 
		/*add and remove socket file descriptors to and from the EPOLL(i'm not a fan poll over select) list*/
