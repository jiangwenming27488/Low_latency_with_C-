...
namespace Common
{
	...
	{
		epoll_event ev{EPOLL | EPOLLIN, {reinterpret_cast<void *>(socket)}}; 
		return !epoll_ctl(epoll_fd_, EPOLL_CTL_ADD, socket->socket_fd, &ev); 
	}
	...
	/*Publish ongoing data from the send buffer and read incoming data from the receive buffer. */
	...
	auto TCPServer::poll() noexcept -> void 
	{
		...
		const int n = epol_wait(epoll_fd_, events_, max_events, 0);
		...
		for (int i = 0; i < n ; ++i)
		{
			...
			if (event.events & EPOLLOUT)
			{
				...
				if (std::find(send_sockets_.begin(), send_sockets_.end(), socket) == send_sockets_.end()) send_sockets_.push_back(socket); 
			}
		...
		while (have_new_connection)
		{
			...
			ASSERT(setNonBlocking(fd) && disabledNagle(fd), "Failed to set non-blocking or no-delay on socket:" + std:to_string(fd)); 

			logger_.log("%:% %() % accept socket:%\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_), fd); 
