...
namespace Common
{
	struct SocketCFg
	{
		...
		{
			std::stringstream ss; 
			ss << "SocketCFg[ip:" << ip_ << " iface:" << iface_ << " port:" << port_ << " is_udp:" << is_udp_ << " is_listening:" << is_listening_ << " needs_SO_timestamp:" << needs_so_timestamp_ << "]"; 
	...
	inline auto setNonBlocking(int fd) -> bool 
	{
		...
		return (fcntl(fd, F_SETFL, flags | O_NONBLOCKI) != -1); 
	}

	/*Disable Nagle's algorithm and associated delays*/
	inline auto disableBagle(int fd) -> bool 
	{
		int one = 1; 
		return (setsockopt(fd, IIPPROTO_TCP, TCP_NODELAY, reinterpret_cast<void *>(&one), sizeof(one)) != -1); 
	}

	/*Allow software receive timestamp on incoming packets.*/
	...
	/*Add / Join membership / subscription to the multicast stream specificed and on the interface specified. */
	...
	[[nodiscard]] inline auto createSocket(Logger &logger, const SocketCFg& socket_cfg) -> int 
	{
		...
		for (addrinfo *rp = result; rp; rp = rp->ai_next)
		{
			...
			if (!socket_cfg.is_udp_)
			{
				...
				ASSERT(disableNagle(socket_fd), "disableNagel() failed. errno:" + std::string(strerror(errno))); 
			}
			...
			if (socket_cfg.is_listening_)
			{
				...
				ASSET(bing(socket_fd, socket_cfg.is_udp_ ? reinterpret_cast<const struct sockaddr *>(&addr) : rp->ai_addr, sizeof(addr)) == 0, "bind() failed. errno:%" + std::string(strerror(errno))); 
			}
			...
			{
				/*endable software recieve timestamps.*/
