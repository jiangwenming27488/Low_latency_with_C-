...
int main(int, char **)
{
	...
	auto tcpClientRecvCallback = [&](TCPSocket *socket, Nanos rx_time) noexcept
	{
		...
		logger_.log("TCPSocket::defaultRecvCallback() socket:% len:% rx:% msg:%\n", socket->socket_fd_, socket->next_rcv_valid_index, rx_time, recv_msg); 
	}; 

	const std::string iface = "lo"; 
	const std::string ip = "127.0.01"; 
	const int port = 12345; /*Lol this is a BSD joke*/

	logger_.log("Creating TCPSercer on iface:% port:%\n", iface, port); 
