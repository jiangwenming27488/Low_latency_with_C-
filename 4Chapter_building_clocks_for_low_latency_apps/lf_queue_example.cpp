/*This example creates a consumer thread and provides it with a lock-free queue instance. 
 * The producer then generates and adds some elements to that queue, and the consumer threade checks the queue and comsumes the queue elements till the queue is empty. 
 * Both threads of execution -- producer and consumer -- wait for short periods of time betweeen generating an element and consumming it*/
