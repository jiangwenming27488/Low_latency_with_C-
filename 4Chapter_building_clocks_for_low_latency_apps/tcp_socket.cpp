...
namespace Common
{
	...
	auto TCPSocket::connect(const std::string &ip, const std::string &ifac, int port, bool is_listening) -> int 
	{
		/*Note that needs_to_timestamp=true for FIFOSequencer.*/
		...
		socket_attrib_.sin_familey = AF_INET; 

		return socket_fd_; 
	}

	/*Called to publish outgoing from the buffers as well as check for and callback if data is available in the read buffers. */
	auto TCPSocket::sendAndRecv() noexcept -> bool
	{
		...
		if (read_size > 0)
		{
			...
			if (cmsg->cmsg_level == SOL_SOCKET && cmsg->cmsg_type == SCM_TIMESTAMP && cmsg->cmsg_len == CMSG_LEN(sizeof(time_kernel)))
			{
				...
				kernel_time = time_kernel.tv_sec * NANOS_TO_SECS + time_kernel.tv_usec * NANOS_TO_MICROS; 	/*Convert timestampto nanoseconds*/
			}
			const auto user_time = getCurrentNanos(); 

			logger_.log("%:% %() % read socket:% len:% utime:% ktime:% diff:%\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_), socket_fd, next_rcv_valid_index_, user_time, kernel_time, (user_time - kernel_time)); 
