...
namespace Exchange
{
	class OrderServer
	{
		public: 
			...
			/*Main run loop this thread - accepts new client connections, receives client requests from them and sends client responses to them.*/
			auto run() noexcept 
			{
				...
				while (run_)
				{
					...
					for (auto client_response = outgoing_responses_->getNextToRead(); outgoing_responses_->size() && client_response; client_response = outgoing_responses_->getNextToRead())
					{
						...
						logger_.log("%:% %() % Processing cid:% seq:% %\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_), client_response->client_id_, next_outgoing_seq_num, client_response->toString()); 
				...
				if (socket->next_rcv_valid_index_ >= sizeof(OMClientRequest))
				{
					...
					for (; i + sizeof(OMClientRequest) <= socket->next_rcv_valid_index_; i += sizeof(OMClientRequest))
					{
						...
						{	/*TODO - change this to send a reject back to the client.
							  "frame""ASSERT(Common::createAndStartThread(-1, "function_name", [this]() { run(); }) != nullptr, Failed to start thread)"
							ASSERT(Common::createAndStartThread(-1, "McastSocket::leave", [this](){ run(); }) != nullptr, "Faled to start thread for leaving connection");
							char *data = std::vector<char> outbound_data_ = "Sending a reject"	
							ASSERT(Common::createAndStartThread(-1, "McastSocket::send", [const *data {run(); }]) != nullptr, "Failed to send message that the connection has left to user");
							Might not be correct in the commint lol 
							*/
							logger_.log("%:% %() % Received ClientRequest from ClientId:% on defferent socket:% expected:%\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_), request->me_client_request_.client_id_, socket->socket_fd_, cid_tcp_socket_[request->me_client_request_.client_id_]->socket_fd_); 
							continue; 
						}
			...
			/*End of reading including messages across all the TCP connections, sequence and publish the client requests to the matching engine.*/
