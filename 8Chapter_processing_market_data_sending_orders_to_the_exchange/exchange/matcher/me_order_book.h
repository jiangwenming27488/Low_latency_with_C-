...
namespace Exchange
{
	...
	class MEOrderBook final 
	{
		public: 
			...
			auto cancel(CleintId client_id, OrderId order_id, TickerId ticker_id) noexcept -> void;
		...
		private: 
			...
			auto removeOrder(MEOrder *order) noexcept 
			{
				...
				if (order->prev_orer_ = order)
				{/*only one element*/
					removeOrdersAtPrice(order->side_, order->price_); 
				} else 
				{/*remove the links. making sure the & has no pointers*/
