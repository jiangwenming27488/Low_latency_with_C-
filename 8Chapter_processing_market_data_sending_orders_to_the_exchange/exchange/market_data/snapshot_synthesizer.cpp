...
namespace Exchange
{
			...
			case MarketUpdateType::ADD:
				{
					...
					order->at(me_market_update.order_id_) = order_pool_.allocate(me_market_update); 
				}
				break; 
			...
			case MarketUpdateType::CANCEL: 
				{
					...
					ASSERT(order != nullptr, "Received:" + me_market_update.toString() + " but order does not exist."); 
					ASSERT(order->order_id_ == me_market_update.order_id_, "Expecting existing order to match new one."); 
					ASSERT(order->side_ == me_market_update.side_, "Expecting exitsing order to match new one."); 
	...
	auto SnapshotSynthesizer::publishSnapshot()
	{
		...
		const MDPMarketUpdate start_market_update{snapshot_size++, {MarketUpdate::SNAPSHOT_START, last_inc_seq_num_}}; 
