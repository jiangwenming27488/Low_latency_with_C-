...
namespace Exchange
{
	class OrderServer
	{
		public: 
			...
			/*Main run loop for this thread - accepts new connections, receives client requests from  them and sends client responses to them*/
			auto run() noexcept
			{
				...
				if (socket->next_rcv_valid_index_ >= sizeof(OMClientRequest))
				{
					...
					for (; i + sizeof(OMClientRequest) <= socket->next_rcv_valid_index_; i += sizeof(OMClientRequest))
					{
						...
						{/*TODO - change this to send a reject back to the client*/
							logger_.log("%:% %() % Received ClientRequest from ClientId:% on deffernet socket:% expected:%\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_), reqest->me_client_request_.client_id_, socket->socket_fd_, cid_tcp_socket_[request->me_client_request_.client_id_]->socket_fd_); 
						...
						{/* - change this to send reject back to the client*/
		...
		private: 
			...
			std::array<size_t, ME_MAX_CLIENTS> cid_next_exp_seq_num_; 
			...
			/*FIFO sequencer responsible for making sure incoming requests are processed in the order in which they were received*/
