...
int main(int, char **)
{
	...
	Exchange::ClientResponseLFQueue client_responses(ME-MAX_CLIENT_UPDATES); 
	...
	const std::string snap_pub_ip = "233.252.12.1", inc_pub_ip = "233.252.12.3"; 
	const int snap_pub_port = 2000, inc_pub_port = 20001; 
	...
	logger->log("%:% %() % Starting Order Server...\m", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str)); 
	order_server = new Exchange::OrderServer(&cliet_requests, &client_reponses, order_gw_iface, order_gw_port); 
	order_server->start(); 
