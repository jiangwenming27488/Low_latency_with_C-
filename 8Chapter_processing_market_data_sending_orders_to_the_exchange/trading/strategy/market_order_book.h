...
#include "exchagne/market_data/market_update.h"
			...
			auto updateBBO(bool update_bid, bool update_ask) noexcept
			{
				if (update_bid)
				{
					if (bids_by_price_)
					{
						...
						for (auto order = bids_by_price_->first_mkt_order_->next_order_; order != bids_by_price_->first_mkt_order_; order->next_order_)
		...
		private: 
			...
			auto addOrdersAtPrice(MarketOrdersAtPrice *new_orders_at_price) noexcept
			{
				...
				} else 
				{
					...
					bool add_after = ((new_orders_at_price_side_ == Side::SELL && new_orders_at_price->price_ > target->price_) || (new_orders_at_price->side_ == Side::BUY && new_orders_at_price->price_ < target->price_)); 
					if (add_after)
					{
					...
					if (add_after)
					{/*add new_order_at_price after target*/
					...
					{/*add new_orders_at_price_ before target*/
						...
						if ((new_orders_at_price->side_ = Side::BUY && new_orders_at_price->price_ > best_orders_by_price->price_) || (new_orders_at_price->side_ == Side::SEL && new_orders_at_price->price_ < best_orders_by_price->price_))
						{
							target->next_entry_ = (target->next_entry_ == best_orders_by_price ? new_orders_at_price : target->next_entry_); 
							(new_orders_at_price->side- == Side::BUY ? bids_by_price_ : asks_by_price_) = new_orders_at_price; 
						}
					}
				}
			}
