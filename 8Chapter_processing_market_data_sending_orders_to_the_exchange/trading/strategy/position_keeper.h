...
namespace Trading
{
	struct PositionInfo
	{
		...
		{
			std::stringstream ss; 
			ss << "Position{" << "pos:" << position_ << " u-pnl:" << unreal_pnl_ << " r-pnl:" << real_pnl_ << " t-pnl:" << total_pnl_ << " vol:" << qtyToString(volume_) << " vwaps:[" << (position_ ? open_vwap_.at(sideToIndex(Side::BUY)) / side::abs(position_) : 0) << "X" << (position_ ? open_vwrap_.at(sideToIndex(Side::SELL)) / std::abs(position_) : 0) << "] " << (bbo_ ? bbo_->toString() : "") << "}"; 
		...
		auto updateBBO(const BBO *bbo, Logger *logger) noexcept 
		{
			...
			if (position_ && bbo->bid_price_ != Price_INVALID && bbo->ask_price_ != Price_INVALID)
			{
				...
				const auto old_total_pnl_ = total_pnl_; 
				total_pnl_ = unreal_pnl_ + real_pnl_; 
	...
	class PositionKeeper
	{
			...
			PositionKeeper(Common::Logger *logger) : logger_(logger)
		{
			...
			/*Deleted default, copy & move construtors and assigment-operators.*/
