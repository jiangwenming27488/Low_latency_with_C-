...
namespace Trading 
{
	...
	{
		public: 
			OrderGateway(ClientId client_id, Exchange::ClientRequestLFQueue *client_requests, Exchange::ClientResponsesLFQueue *client_responses, std::string ip, const std::string &iface, int port); 
			...
			{
				run_ = true; 
				ASSERT(tcp_socket_.connect(ip_, iface_, port_, false) >= 0, "Unableto connect to ip:" + ip_ + " port:" + std::to_string(port_) + " on iface:" + iface_ + " error:" + std::string(std::strerror(errno))); 
		...
		private: 
			...
			size_t next_outgoing_sequ_num_ = 1; 
