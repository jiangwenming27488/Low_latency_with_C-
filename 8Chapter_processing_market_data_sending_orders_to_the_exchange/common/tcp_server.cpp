...
namespace Common
{
	...
	auto TCPServer::listen(const std::string &iface, int port) -> void
	{
		...
		ASSERT(epoll_fd_ >= 0, "epoll_create() failed error:" + std::string(std::sterror(errno))); 
	...
	auto TCPServer::sendAndRecv() noexcept -> void 
	{
		...
		if (recv) /*There were some events and they all been dispatched, inform listener.*/
	...
	auto TCPServer::poll() noexcept -> void
	{
		...
		for (int i = 0; i < n; ++i)
		{
			...
			if (event.events & EPOLLIN)
			{
				...
				if (std::find(receive_sockets_.begin(), receive_sockets_.end(), socket) == receieve_sockets_.end())
		...
		while (have_new_connection)
		{
			...
			if (std::find(receive_sockets_.begin(), receive_sockets_end(), socket) == receive_sockets_.end())
				receive_sockets_.push_back(socket); 
		}
	}
}
