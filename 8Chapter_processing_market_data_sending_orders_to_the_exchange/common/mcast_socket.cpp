...
namespace Common
{
	...
	/*Add / Join memebership / subscription to a multicast stream.*/
	...
	auto McastSocket::sendAndRecv() noexcept -> bool 
	{
		...
		const ssize_t n_rcv = recv(socket_fd_, inbound_data() + next_rcv_valid_index_, McastBufferSize - next_rcv_valid_index_, MSG_DONTWAIT); 
		if (n_rcv > 0)
		{
			next_.log("%:% %() % read socket:% len:%\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_), socket_fd_, next_rcv_valid_index_); 
			recv_callback_(this); 
		}
	...
	/*Copy data to send buffers - doe not send them out yet*/
	auto McastSocket::send(const void *data, size_t len) noexcept -> void
	{
		...
		ASSERT(next_send_valid_index_ < McastBuffersSize, "Mcast socket buffer filled up and sendAndRecv() not called "); 
	}
}
