...
namespace Common
{
	...
	constexpr size_t ME_MAX_MAKRET_UPDATES = 256 * 1024; 
	...
	struct RiskCfg
	{
		...
		auto toString() const 
		{
			...
			ss << "RiskCfg{" << "max-order-size:" << qtyToString(max_order_size_) << " " << "max-position:" << qtyToString(max_position_) << " " << "max-loss:" << max_loss_ << "}"; 
	...
	typedef std::array<TradeEngineCfg, ME_MAX_TICKERS> TradeEngineCfgHashMap; /*PLEASE UNDERSTAND THIS IS A IMPORTANT HASH MAP OBJECT*/
}
