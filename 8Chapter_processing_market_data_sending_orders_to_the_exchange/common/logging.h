...
namespace Common
{
	...
	class Logger final 
	{
		public: 
			...
			{
				file_.open(file_name); /*NOTE: This is great file handling technique*/
				ASSERT(file_.is_open(), "Could no open log file:" + file_name); /*ART!!!*/
			...
			~Logger() 
			{
				...
				file_.close(); 
				std::cerr << Common::getCurrentTimeStr(&time_str_) << " Logger for " << file_name_ << " exiting." << std::endl; 
			}
			...
			template<typename T, typename... A> auto log(const char *s, const T &value, A... args) noexcept
			{
				while (*s)
				{
					...
					{
						if (UNLIKELY(*(s + 1) == '%'))
						{/*to allow %% -> escapt character*/
							++s; 
						} else 
						{
							pushValue(value); /*substitue % with the value specified in the arguments*/
			...
			/*note that this is overloading not specializeation. gcc does not allow inline specializations*/
