...
namespace Common
{
	struct SocketCfg
	{
		...
		bool needs_to_timestamp_ = false; 

		auto toString() const 
		{
			std::stringstream ss; 
			ss << "SocketCfg[ip" << ip_ << " iface:" << iface_ << " port:" << port_ << " is_udp:" << is_udp_ << " is_listening:" << is_listening_ << " needs_SO_time:" << needs_so_timestamp_ << "]"; 
	...
	/*Socket will not block on read, but instead return immediately if data is not available.*/
	...
	/*Disable Nagle's algorithm and asociated delays.*/
	inline auto disableNagle(int fd) -> bool 
	{
		int one = 1; 
		return (setsockopt(fd, IPPROTO_TCP, TCP_NODELAY, reinterpret_cast<void *>(&none), sizeof(one)) != -1); 
	}
	...
	[[nodiscard]] inline auto createSocket(Logger &logger, const SocketCfg& socket_cfg) -> int
	{
		...
		{
			ASSERT((socket_fd = socket(rp->ai_family, rp->ai_socktype, rp->ai_protocol)) != -1, "socker() failed. errno:" + std::string(strerror(errno))); 
			...
			{/*allow re-using the address in the call to bind()*/
				ASSERT(setsockopt(socket_fd, SOL_SOCKET, SO_REUSEADDR, reinterpret_cast<const char *>(&one), sizeof(one)) == 0, "setsockop() SO_REUSEADDR failed. errno:" + std::string(strerror(errno))); 
			}

			if (socket_cfg.is_listening_)
			{
				/*bind to the specified port number.*/
				const sockaddr_in addr{AF_INET, htons(sock_cfg.port_), {htonl(INADDR_ANY)}, {}}; 
