...
namespace Exchange
{
	...
	class UnorderedMapMEOrderBook final 
	{
		public: 
			...
			/*Create and add a new order in the order book with providing attributes.
		...
		private: 
			...
			/*The parent matching engine instance, used to publish market data and client repsonses.*/
		...
		private: 
			 ...
			 * It will return remaining quantity on the aggressive order in the "leaves_qty" parametr*/
			auto match(TickerId ticker_id, ClientId cleint_id, Side side, OrderId client_order_id, OrderId new_market_order_id, MEOrder* bid_itr, Qty* leaves_qty) noexcept; 

			/*Check if a new order with the provided attributes would match against existing passive orders on the other side of the other book.
