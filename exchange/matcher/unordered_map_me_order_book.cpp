...
namespace Exchange
{
	UnorderedMapMEOrderBook::UnorderedMapMEOrderBook(TickerId ticker_id, Logger *logger, MatchingEngine *matching_engine) : ticker_id_(ticker_id), matching_engine_(matching_engine), orders_at_price_pool_(ME_MAX_PRICE_LEVELS), order_pool_(ME_MAX_ORDER_IDS), logger_(logger)
	{
	...
	/*Match new aggressive order with the provided parameters against a passive order held in the "bid_itr" object and generate client responses and market updates for the match.
