...
namespace Exchange 
{
	...
	class MEOrderBook final 
	{
		public: 
			 ...
			 * It will check to see if this new order matches and existing passive order with opposite side, and perform the matching if this is the case.*/
			auto add(ClientId client_id, OrderId client_order_id, TickerId ticker_id, Side side, Price price, Qty qty) noexcept -> void; 

			/*Attempt to canel an order in the order book, issue a "cancel-reject" if order does not exits.*/
		...
		private: 
			...
			/*The parent matching engine instance, used to publish market data and client reponses. */
			...
			/*Pointer to beginning / best price / top of book of buy and sell price levels*/
		...
		private: 
			...
			/*Add a new "MEOrdersAtPrice" at the correct price into the containers - the hash map and doubly linked list of price levels*/
			 ...
			 * It will return remaining quantity on aggressive order in the "leaves_qty" parameter.*/
