...
namespace Exchange
{
	class SnapshotSynthesizer
	{
		public: 
			...
			/*Main method for this thread - process incremental updates from the market publisher, updates the snapshot and publishes the snapshot periodically*/
			auto run() -> void; 

			/*Deleted default, copy and (didnt want to use &) move constuctors and assignment-operators.*/
		...
		private: 
			...
			/*Hash map from "TickerId" -> Full limit order book snapshot containing infromation for every live order.*/
			...
			/*Memory pool to mange "MEMarketUpdate" messages for the orders in the snapshot limit order books.*/
