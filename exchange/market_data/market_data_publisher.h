...
namespace Exchange 
{
	class MarketDataPublisher
	{
		public: 
			...
			/*Main run loop for this thread - consumes market updates from the lock free queue from the matching engine, publishes them on the incrementl multicast stream and forwards them to the snapshot synthesizer.*/
			auto run() noexcept -> void; 

			/*Deleted default, copy & move constructors and assginment-operators.*/
		...
		private: 
			...
			/*Multicast socket to represent the incremtal market data stream.*/
			Common::McastSocket incremental_socket_; 

			/*Snapshot synthesizer which synthesizes and publishes limit order book snapshot multicast stream*/
