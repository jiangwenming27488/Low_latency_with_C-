...
namespace Exchange
{
	SnapshotSynthesizer::SnapshotSynthesizer(MDPMarketUpdateLFQueue *market_updates, const std::string &iface, const std::string &snapshot_ip, int snapshot_port) : snapshot_md_updates_(market_updates), logger_("exchange_snapshot_synthesizer.log"), snapshot_socket_(logger_), order_pool_(ME_MAX_ORDER_IDS)
	{
	...
	{
		run_ = true; 
		ASSERT(Common::createAndStartThread(-1, "Exchange/SnapshotSynthesizer", [this]() { run(); }) != nullptr, "Faild to start SnapshotSynthesizer thread. "); 
	}
	...
	/*Process an incremtal market update and update the limit order book snapshot*/
	auto SnapshotSynthesizer::addToSnapshot(const MDPMarketUpdate *market_update)
	{
		...
		ASSERT(market_update->seq_num_ == last_inc_seq_num_ + 1, "Expected incremetal seq_nums to increase."); 
		last_inc_seq_num_ = market_update->seq_num_; 
	}

	/*Publish a full snapshot cycle on the snpashot multicast stream.*/
	auto SnapshotSynthesizer::publishSnapshot() 
	{
		...
		for (size_t ticker_id = 0; ticker_id < ticker_orders_.size(); ++ticker_id)
		{
			...
			/*We start order information for each instrument by first publishing "CLEAR" message so the downstream consumer can clear the order book.*/
		...
		/*The snapshot cycle ends with a "SNAPSHOT_END" message and "order_id_" constains the last sequence number from the incremental market data stream used to build this snapshot.*/
	...
	/*Main method for this thread - processes incremental updates from the market data publisher, updates the snapshot and publishes the snaphot peroidically.*/
	void SnapshotSynthesizer::run() 
	{
		...
		while (run_)
		{
			...
			if (getCurrentNanos() - last_snapshot_time_ > 60 * NANOS_TO_SECS)
			{
				...
				publishSnapshot();	/*Please understand that most of the work is being done by the "publishSnapshot()" method its even calling the socket library most important method.*/ 
			}
		}
	}
}
