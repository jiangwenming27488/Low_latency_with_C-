...
namespace Common
{
	...
	auto TCPServer::addToEpollList(TCPSocket *socket)
	{
		...
		return !epoll_ctl(epoll_fd, EPOOL_CTL_ADD, socket->socket_fd_, &ev); 
	}
	...
	{
		const int max_event = 1 + send_sockets_.size() + receive_sockets_.size(); 
		...
		while (have_new_connection)
		{
			...
			int fd = accept(listener_socket_.socket_fd_, reinterpret_cast<sockaddr *>(&add), &addr_len); 
			if (fd == -1)
				break; 
