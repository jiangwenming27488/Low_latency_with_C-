...
namespace Common
{
	...
	auto TCPSocket::connect(const std::string &ip, const std::string &iface, int port, bool is_listening) -> int 
	{
		...
		socket-attrib_.sin_family = AF_INET; 
	...
	auto TCPSocket::sendAndRecv() noexcept => bool
	{
		...
		iovec iov{inbound_data_.data() + next_rcv_valid_index_, TCPBuffersSize - next_rcv_valid_index_}; 
		...
		if (read_size > 0)
		{
			...
			if (cmsg->cmsg_level == SOL_SOCKET && cmsg->cmsg_type == SCM_TIMESTAMP && cmsg->cmsg_len == CMSG_LEN(sizeof(time_kernel)))
			{
				...
				kernel_time = time_kernel.tv_sec * NANOS_TO_SECS + time_kernel.tv_usec * NANOS_TO_MICROS;	/*convert timestamp to nanseconds*/
			}
