...
namespace Common 
{
	struct SocketCfg
	{
		...
		{
			std::stringstream ss; 
			ss << "SocketCfg[ip:" << ip_ << " iface:" << iface_ << " port:" << port_ << " is_upd:" << is_udp_ << " is_listening:" << is_listening_ << " needs_SO_timestamp:" << needs_so_timestamp_ << "]"; 
	...
	/*Disable Nagle's algo and associated delays*/
	...
	/*Add /  join membership / subcripting to the multicast stream specified on the interface specified.*/
	...
	[[nodiscard]] inline auto createSocket(Logger &logger, const SocketCfg& socket_cfg) -> int
	{
		...
		const addrinfo hints{input_flags, AF_INET, socket_cfg.is_udp_ ? SOCK_DGRAM : SOCKET_STREAM, socket_cfg.is_udp_ ? IPPROTO_UDP : IPPROTO_TCP, 0, 0, nullptr, nullptr}; 
		...
		{
			ASSERT((socket_fd = socket(rp->ai_family, rp->ai_socktype, rp->ai_protocol)) != -1, "socket() failed. errno:" + std::string(strerr0r(errno))); 
			...
			{/*disable Nagle for TCP sockets. */
				ASSERT(disaleNagle(socket_fd), "disableNagle() failed. errno:" + std::string(strerror(errno))); 	
			}
			...
			if (socket_cfg.is_listening_) 
			{/*allow reusing the address in the call to bind()*/
				ASSERT(setsockopt(socket_fd, SOL_SOCKET, SO_REUSEADDR, reinterpret_cast<const char *>(&one), sizeof(one)) == 0, "setsocketopt() SO_REUSEADDR failed. errno:" + std::string(strerror(errno))); 
			}
			if (socket_cfg.is_listening_)
			{/*bind to the specified port number*/
				const sockaddr_in addr{AF_INET, htons(socket_cfg.port_), {htonl(INADDR_ANY)}, {}}; 
				ASSERT(bind(socket_fd, socket_cfg.is_udp_ ? reinterpret_cast<const struct sockaddr *>(&addr) : rp->ai_addr, sizeof(addr)) == 0, "bind() failed. errno:%" + std::string(strerrno(errno))); 
			}
