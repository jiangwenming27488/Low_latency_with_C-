...
namespace Exchange
{
	...
	inline std::string cleintRequestTypeToString(ClientRequestType type)
	{
		switch (type)
		{
			case ClinetRequestType::NEW: 
				return "NEW"; 
...
#pragma pack(push, 1)
	...
	struct MEClientRequest
	{
		...
		ClientId cliet_id_ = ClientId_INVALID; 
	...
	struct OMClientRequest
	{
		...
		{
			std::stringstream ss; 
			ss << "OMClientRequest" << " [" << "seq:" << seq_num_ << " " << me_client_request_.toString() << "]"; 
			return ss.str(); 
		}
	}; 
