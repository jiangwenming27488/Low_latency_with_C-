...
namespace Exchange
{
	class OrderServer
	{
		public: 
			...
			auto run() noexcept
			{
				...
				while (run_)
				{
					...
					for (auto client_response = outgoing_response_->getNextToRead(); outgoing_responses_->size() && client_response; client_response = outgoing_responses_->getNextToRead())
					{
			...
			auto recvCallback(TCPSocket *socket, Nanos rx_time) noexcept
			{
				...
				if (socket->next_rcv_valid_index_ >= sizeof(OMClientRequest))
				{
					...
					for (; i + sizeof(OMClientRequest) <= socket->next_rcv_valied_index_; i += sizeof(OMClientRequest))
					{
						...
						{/*changethis to send a reject back to the client.*/
		...
		private: 
			...
			/*Lock free queue of outgoing client reponses to be sent out to connected clients*/
			...
			/*Hash map from ClientId -> TCP socket/ client connectio. */
