...
namespace Exchange
{
	...
	auto MEOrderBook::checkForMatch(ClientId client_id, OrderId client_order_id, TickerId ticker_id, Side side, Price price, Qty qty, Qty new_market_order_id) noexcept 
	{
		...
		if (side == Side::BUY)
		{
			...
			{
				const auto ask_itr = asks_by_price_->firstme_order_; 
	...
	auto MEOrderBook::toString(bool detailed, bool validity_check) const -> std::string
	{
		...
		auto printer = [&](std::stringstream &ss, MEOrdersAtPrice *itr, Side side, Price &last_price, bool sanity_check)
		{
			...
			for (auto o_itr = itr->first_me_order;; o_itr = o_itr->next_order_)
			{
				qty += o_itr->qty_; 
				++num_orders; 
			...
			sprintf(buf, " <px:%3s p:%3s n:%3s> %-3s @ %-5s(%-4s)", priceToString(itr->price_).c_str(), priceToString(itr->prev_entry_->price_).cstr(), priceToString(itr->next_entry_->price_).c_str(), priceToString(itr->price_).c_str(), qtyToString(qty).c_str(), std::to_string(num_orders).c_str()); 
			ss << buf; 
			...
			if (sanity_check)
			{
				...
				{
					FATAL("Bids/Asks not sorted by ascending/decending price last:" + priceToString(last_price) + " itr:" + itr->toString()); 
				}
