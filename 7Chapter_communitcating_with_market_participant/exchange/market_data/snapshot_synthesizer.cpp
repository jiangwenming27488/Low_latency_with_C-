...
namespace Exchange
{
	SnapshotSynthesizer::SnapshotSynthesizer(MDPMarketUpdateLFQueue *market_updates, const std::string &iface, const std::string &snapshot_ip, int snapshot_port) : snapshot_md_updates_(market_updates), logger_("exchange_snapshot_synthesizer.log"), snapshot_socket_(logger_), order_pool_(ME_MAX_ORDER_IDS)
	{
		...
		for (auto& order : ticker_orders_)
			orders.fill(nullptr); 
	}

	SnapshotSynthesizer::~SnapshtoSynthesizer()
	{
		stop(); 
	}
	...
	auto SnapshotSynthesizer::addToSnapshot(const MDPMarketUpdate *market_update)
	{
		...
		switch (me_market_update.type_)
		{
			...
			case MarketUpdateType::MODIFY: 
				{
					auto order = orders->at(me_market_update.order_id_); 
					ASSERT(order != nullptr, "Received:" + me_market_update.toString() + " but order does not exitst."); 
					ASSERT(order->order_id_ == me_market_update.order_id_, "Expecting existing order to match new one."); 
					ASSERT(order->side_ == me_market_update.side_, "Expecting exitsting order to match new one."); 
	...
	auto SnapshotSynthesizer::publishSnapshot()
	{
		...
		const MDPMarketUpdate start_market_update{snapshot_size++, {MarketUpdateType::SNAPSHOT_START, last_inc_seq_num}}; 
		logger_.log("%:% %() %%\n", __FILE__, __LINE__, __FUNCTION__, getCurrentTimeStr(&time_str_), start_market_update.toString()); 
		snapshot_socket_.sent(&start_market_update, sizeof(MDPMarketUpdate)); 
		...
		for (size_t ticker_id = 0; ticker_id < ticker_orders_.size(); ++ticker_id)
		{
			...
			const MDPMarketUpdate clear_market_update{snapshot_size++, memarket_update}; 
	...
	void SnapshotSynthesizer::run()
	{
		...
		while (run_)
		{
			...
			{
				logger_.log("%:% %() % Processing %\n", __FILE__, __LINE__, FUNCTION__, getCurrentTimeStr(&time_str_), market_update->toString().c_str()); 
