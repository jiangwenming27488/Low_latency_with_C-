...
/*"./trading_main" CLIENT_ID ALGO_TYPE [CLIP_1 THRESH_1 MAX_ORDER_SIZE_1 MAX_POS_1 MAX_LOSS_1] [CLIP_2 MAX_ORDER_SIZE_2 MAX_POS_2 MAX_LOSS_2]...*/
int main(int argc, char **argv)
{
	if(argc < 3)
	{
		FATAL("USAGE trading_main CLIENT_ID ALGO_TYPE [CLIP THRESH_1 MAX_ORDER_SIZE_1 MAX_POS_1 MAX_LOSS_1] [CLIP_2 THRESH_2 MAX_ORDER_SIZE_2 MAX_POS_2 MAX_LOSS_2] ..."); 
	}
	...
	/*The lock free queue to facilitate communication between order gateway <-> trade engine and market data consumer -> trade engine*/
	...
	{
		ticker_cfg.at(next_ticker_id) = {static_cast<Qty>(std::atoi(argv[i])), std::atof(argv[i + 1]), {static_cast<Qty>(std::atoi(argv[i + 2])), static_cast<Qty>(std::atoi(argv[i + 3])), std::atof(argv[i + 4])}}; 
	}
	...
	/*For the random trading algorithm, we simply implement it here instead of creating a new trading algorithm which i sanother possibility.
	 * Generate random orders with rand attributes and ranomly cancel some of them. */
	if (algo_type == AlgoType::RANDOM)
	{
		...
		for (size_t i = 0; i < 10000; ++i)
		{
			...
			{
				logger->log("%:% %() % Stopping early because been silent for % seconds...\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str), trade_engine->silentSeconds()); 
