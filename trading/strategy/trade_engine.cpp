...
namespace Trading 
{
	TradeEngine::TradeEngine(Common::ClientId client_id, AlgoType algo_type, const TradeEngineCfgHashMap &ticker_cfg, Exchange::ClientRequestLFQueue *client_requests, Exchange::ClientResponseLFQueue *client_responses, Exchange::MEMarketUpdateLFQueue *market_updates) : client_id_(client_id), outgoing_ogw_requests_(client_requests), incoming_ogw_responses_(client_responses), incoming_md_updates_(market_updates), logger_("trading_engine_" + std::to_string(client_id) + ".log"), feature_engine_(&logger_), position_keeper_(&logger_), order_manager_(&logger_, this, risk_manager_), risk_manager_(&logger_, &position_keeper_, ticker_cfg)
	{
		...
		/*Create the trading algo instance based onthe "AlgoType" provided
		...
		{
			logger_.log("%:% %() % Initialized % Ticker:% %\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_), algoTypeToString(algo_type), i, ticker_cfg.at(i).toString()); 
		}
	}
	...
	{
		logger_.log("%:% %() % Sending %\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_), client_request->toString().c_str()); 
		...
		*next_write = std::move(*client_request); 
		...
		TTT_MEASURE(T10_TradingEngine_LFQueue_write, logger_); 
	}

	/*Main loop for this thread - process incoming client responses and market data updates which in turn my generate client requests.*/
	auto TradeEngine::run() noexcept -> void
	{
		...
		while (run_)
		{
			...
			{
				TTT_MEASURE(T9t_TradingEngine_LFQueue_read, logger_); 

				logger_.log("%:% %() % Processing%\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_), client_response->toString().c_str()); 
			...
			{
				TTT_MEASURE(T9_TradingEngine_LFQueue_read, logger_); 

				logger_.log("%:% %() % Processing %\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_), market_update->toString().c_str()); 
	...
	/*Process changes to the order book - updates the position keeper, feature engine and informs the trading algo about the update*/
	...
	/*Process trade events - updates the feature engine and informs the trading algo about the trade event*/
	...
	/*Process client responses - updates the position keeper and informs the trading algo about the response*/
