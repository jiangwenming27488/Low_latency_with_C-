...
namespace Trading
{
	...
	{
		public: 
			TradeEngine(Common::ClientId client_id, AlgoType algo_type, const TradeEngineCfgHashMap &ticker_cfg, Exchange::ClientRequestLFQueue *client_requests, Exchange::ClientResponseLFQueue *client_responses, Exchange::MEMarketUpdateLFQueue *market_updates); 
			...
			auto stop() -> void 
			{
				...
				{
					logger_.log("%:% %() % Sleeping till all updates are consumed ogw-size:% md-size:%\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_), incoming_ogw_responses_->size(), incoming_md_updates_->size()); 

					using namespace std::literals::chrono_literals;
					std::this_thread::sleep_for(10ms); 
				}

				logger_.log("%:% %() % POSITIONS\n%\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_), position_keeper_.toString()); 

				run_ = false; 
			}

			/*Main loop for this thread - processes incoming client responses and market data updates which in turen may generate client requets */
			auto run() noexcept -> void; 

			/*Write a client request to the lock free queue for the order server to consume and send the exchange.*/
			...
			/*Process trade events - updates the features engine and informs the trading algorithm about the trade event*/
			...
			/*Function wrappers to dispatch order updates, trade events and client responses to the trading algorithm*/
			...
			/*Deleted defualt, copy & move constructors and assignment-operators*/
			TradeEngine() = delete; 
		...
		private: 
			...
			/*Hash map container form "TickerId" -> "MarketOrderBook"*/
			 ...
			 * Second to consume incoming client responses from, written to by the order gateway basedon data received from the exchange.
			...
			/*Feature engine for the trading algorithm*/
