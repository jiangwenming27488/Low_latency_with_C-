...
namespace Trading 
{
	...
	/*Enumeration that captures the result of the risk check - "ALLOWED" meas it passed risk checks, the other values represent the failure reason. */
	...
	/*Structure that represents the infromation needed for risk checks for a single trading instrument*/
	struct RiskInfo
	{
		...
		/*Check risk to see if we are allowed to send an order of the specified quantity on the specidified side.
		 * WIll return a "RiskCheckResult" value to convoey the output of the risk check*/
		...
		{
			std::stringstream ss; 
			ss << "RiskInfo" << "[" << "pos:" << position_info_->toString() << " " << risk_cfg_.toString() << "]"; 
	...
	class RiskManager
	{
		public: 
			...
			/*Deleted default, copy & move consturctors and assignmen-operators*/
			RiskManager() = delete; 
