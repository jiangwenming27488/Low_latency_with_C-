...
namespace Trading 
{
	...
	/*Manages order for a trading algorithm, hides the complexity of order management to simplify trading strategies*/
	class OrderManager
	{
		public:
			OrderManager(Common::Logger *logger, TradeEngine *trade_engine, RiskManager& risk_manager) : trade_engine_(trade_engine), risk_manager_(risk_manager), logger_(logger)
		{
			...
			/*Move a singel order on the specified side so that it has the specified price and quantity.
			...
			/*Have order of quantity clip at the specified buy and sell prices.
		 	* This can result in new orders being send if there are none.
		 	* This can result in existing orders being cancelled if they are not at the specified price of of the specified quantity.
		 	* Specifying "Price_INVALID" for the buy or sell price indicates that we do not want an order there.*/
			...
			/*Helper method to featch the by and sell "OMOrders" for the specified "TickerId"*/
			auto getOMOrderSideHashMap(TickerId ticker_id) const 
			{
				return &(ticker_side_order_.at(ticker_id)); 
			}

			/*Deleted default, copy & move constructors ans assignment_operators.*/
			OrderManager() = delete; 
		...
		private:
			...
			/*Risk manager to perform pre-trade risk check */
