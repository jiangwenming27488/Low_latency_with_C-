...
namespace Trading
{
	class MarketDataConsumer 
	{
		...
		private: 
			...
			/*Lock free queue on which decoded market data updates are published to, to be consumed by the trade engine.*/
			...
			/*Tracks if we are currently in the process of recovering / synchroniing with the snapshot market data stream either beacause we just started up or we dropped a packet.*/
			...
			/*Container to queue up market data updates from the snapshot and incremental channels, queued up in order of increasing sequence numbers*/
		...
		private: 
			...
			/*Start the process of snapshot synchronization by subscribing to the snaphot multicast stream*/
			auto startSnapshotSync() -> void; 

			/*Check if a recovery / synchronization is possible from the queue up market data updates from the snapshot and incremental market data stream.*/
