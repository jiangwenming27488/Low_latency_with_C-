...
namespace Trading
{
	...
	/*Main loop for this thread - reads and proceses messages from the multicast sockets - the heavy lefting is the "recvCallback()" and "checkSnapshotSync()" methods*/
	...
	auto MarketDataConsumer::startSnapshotSync() -> void 
	{
		...
		ASSERT(snapshot_mcast_socket_.join(snapshot_ip_), /*IGMP multicast subscription*/ "Join failed on:" + std::to_string(snapshot_mcast_socket_.socket_fd_) + " error:" + std::string(std::strerror(errno))); 
	}

	/*Check if a recovery / synchronization is possible from the queued up market data updates from the snapshot and incremetal market data streams.*/
	auto MarketDataConsumer::checkSnapshotSync() -> void 
	{
		...
		{
			logger_.log("%:% %() % Returning because have not seend a SNAPSHOT_END yet.\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_)); 
			return; 
		}
		...
		logger_.log("%:% %() % Recover % snapshot and % incremental orders.\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_), snapshot_queued_msgs_.size() - 2, num_incrementals); 
		snapshot_queued_msgs_.clear(); 
		incremental_queued_msgs_.clear(); 
		in_recovery_ = false; 

		snapshot_mcast_socket_.leave(snapshot_ip_, snapshot_port_);; /*; I commited out the ';'*/
	}

	/*Queue up a message in the "*_queued_msgs_" containers, first parameter specifies if this update come from the snapshot or the incremenatal streams*/
	...
	/*Process a market data update, the consumer needs to use the socket parameter to figure out whether this come from the snapshot or the incremental stream.*/
	auto MarketDataConsumer::recvCallback(McastSocket *socket) noexcept -> void
	{
		...
		if (socket->next_rcv_valid_index_ >= sizeof(Exchange::MDPMarketUpdate))
		{
			...
			for (; i + sizeof(Exchange::MDPMarketUpdate) <= socket->next_rcv_valid_index_; i += sizeof(Exchange::MDPMarketUpdate))
			{
				...
				logger_.log("%:% %() % Received % socket len:% %\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_), (is_snapshot ? "snapshot" : "incremental"), sizeof(Exchange::MDPMarketUpdate), request->toString()); 
				...
				if (UNLIKELY(in_recovery_))
				{
					...
					{/*if we entered recovery, start the snpashot synchonization proces by subscribing to the snapshot multicast stream*/
					...
					queueMessage(is_snapshot, request); /*queue up the market data update message and check if snpashot recovery / synchronization can be completed successfully.*/
				} else if (!is_snapshot)
