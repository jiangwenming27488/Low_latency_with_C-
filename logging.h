...
namespace Common
{
	...
	/*Rpresents a single and primitive log entry*/
	struct LogElement
	{
	...
	class Logger final
	{
		public: 
			...
			/*Overload methods to write different log entry types to the lock free queue
			...
			template<typename T, typename... A> auto log(const char *s, const T &value, A... args) noexcept 
			{
				while (*s)
				{
					if (*s == '%')
					{
						...
						{
							pushValue(value); 	/*sub % with the value specified in the arg*/
							log(s + 1, args...);	/*pop an argument and call self recuresively*/
							return; 
						}
					}
					pushValue(*s++); 
		...
		private: 
			/*File to wich the log entries will be written*/
			const std::string file_name_; 
			std::ofstream file_; 

			/*Lock free queue of log elements from main logging thread to beackground formatting and disk writer thread*/
