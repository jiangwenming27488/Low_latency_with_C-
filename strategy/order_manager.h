...
namespace Trading
{
	...
	/*Manages order for a trading algoithm, hids the complexity of order manager to simplify trading strategies*/
	class OrderManager
	{
		public: 
			OrderManager(Common::Logger *logger, TradeEngine *trade_engine, RiskManager& risk_manager) : trade_engine_(trade_engine), risk_manager_(risk_manager), logger_(logger)
			{
			...
			/*Send a new order with speified attribute, and update the "OMOrder" object passed here.*/
			auto newOrder(OMOrder *order, TickerId ticker_id, Price price, Side side, Qty qty) noexcept -> void; 

			/*Send cancel for the specified order, and update the "OMOrder" object passed here*/
			auto cancelOrder(OMOrder *order) noexcept -> void; 

			/*Move a single order on te speified side so that it has the specified price quanity
			 * This will perform risk checks prior to sending the order, and update the "OMOrder" objectd passed here*/
			...
			/*Have orders of quantity clip at the specified buy and sell price.
			 * This can result in new orders being sent if there are more
			 * This can result in existing orders being cancelled if they are not at the specified price or of the specified quantity
			 * Specifying "Price_INVALID" for the by or sell prices indiates that we do not want an order there*/
			...
			/*Helper method to fetch the by and sell "OMOrders" for the speified "TickerId"*/
		...
		private:
			...
			/*Risk Manager to perform pre-trade risk check "VERY IMPORTANT"*/
			const RiskManager& risk_manager_; 

			std::string time_str_; 
			Common::Logger *logger_ = nullptr; 

			/*Hash nap container from "TickerId" -> "Side" -> "OMOrder"*/
