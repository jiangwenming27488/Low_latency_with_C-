...
namespace Trading
{
	/*Represents the type / acation in the order structure in the order manager*/
	...
	/*Internal strucure used by the order manager to represent a single strategy order*/
	struct OMOrder
	{
		...
		{
			std::stringstream ss; 
			ss << "OMOrder" << "[" << "tid:" << tickerIdToString(ticker_id_) << " " << "oid:" << orderIdToString(order_id_) << " " << "side:" << sideToString(side_) << " " << "price:" << priceToString(price_) << " " << "qty:" << qtyToString(qty_) << " " << "state:" << OMOrderStateToString(order_state_) << "]"; 
			return ss.str(); 
		}
	}; 
