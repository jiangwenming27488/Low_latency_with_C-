...
namespace Trading 
{
	class LiquidityTaker
	{
		public: 
			...
			/*Process trade events, fetch the aggressive trade ratio from the feature engine, check against the tradig threshold and send aggressive orders.*/
			auto onTradeUpdate(const Exchange::MEMarketUpdate *market_update, MarketOrderBook *book) noexcept -> void 
			{
				...
				{
					logger_->log("%:% %() % % agg-qty-ratio:%\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_), bbo->toString().c_str(), agg_qty_ratio); 
			...
			/*Deleted default, copy & move consturctors ans assignment-operators*/
