...
namespace Exchange 
{
			...
			case MarketUpdateType::MODIFY: 
				{
					...
					ASSERT(order->order_id_ == me_market_update.order_id_, "Expecting exisitng order to match new one."); 
	...
	/*Publish a full snapshot cycle on the market multicast stream.*/
	auto SnapshotSynthesizer::publishSnapshot()
	{
		size_t snapshot_size = 0; 

		/*The snapshot cycle starts with a "SNAPSHOT_START" message and "order_id_" constains the last sequence number from the incremental market data stream used to build this snapshot.*/
		...
		/*Publish order information fro each order in the limit order book for each instrument.*/
		...
		/*THe snapshot cycle ends with the "SNAPSHOT_END" messae and "order_id_" contains the last sequence number from the incremental market data stream used to build this snapshot*/
	...
	/*Main mehtod for this thread - processes incremental updates from the market data publisher, updates the snapshot and publishes the snapshot periodically.*/
