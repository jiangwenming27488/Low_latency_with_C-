...
/*The structures above go over the wire / network, so the binary structures are packed to remove system dependent extra padding.*/
#pragma pack(push, 1)

	/*Market update structre used internally by the matching engine.*/
	...
	struct MDPMarketUpdate
	{
		...
		{
			std::stringstream ss; 
			ss << "MEPMarketUpdate" << " [" << " seq:" << seq_num_ << " " << me_market_update_.toString() << "]"; 
			return ss.str(); 
		}
	}; 
