...
namespace Trading 
{
	class MarketDataConsumer
	{
		public: 
			...
			{
				run_ = true; 
				ASSERT(Common::createAndStartThread(-1, "Trading/MarketDataConsumer", [this]() { run(); }) != nullptr, "Fail to start MarketData thread. "); 
			}
			...
			/*Deleted default, copy & move constructors and assignmet-operators.*/
		...
		private: 
			/*Track the next expected sequene number on the incremental market data stream, used to detect gaps / drops*/
			size_t next_exp_inc_seq_num_ = 1; 

			/*Lock free queue on which decoded market data updates are puhed to, to be consumed by the trade engine.*/
			...
			/*Containers to queue up market data updates from the snapshots and incrematal channels, queued up in order of increasing sequence numbers.*/
		...
		private: 
			/*Main loop for this thread - reads and processes messages from the mulitcast sockets - the heavy lifting is the "recvCallback()" and "checkSnapshotSync()" methods*/
			auto run() noexcept -> void;

			/*Process a market update, the consumer needs to use the socket parameter to figure out whether this came from the snapshot or the incremental stream.*/
			...
			/*Check if a recovery / synchronization is possible from the queued up market data updats from the snapshot and incremetal market data streams*/
