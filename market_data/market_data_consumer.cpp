...
namespace Trading 
{
	...
	/*Main loop for this thread - reads and processes messages from the multicast sockets - the heavy lifting is the "recvCallback()" and "checkSnapshotSyn()" methods*/
	...
	/*Start the proces of snapshot synchronization by subscribing to the snapshot multicast stream*/
	...
	/*Check if a recovery / sychronization is possible from the queued up market data updates from the snapshot and incremental market data streams.*/
	auto MarketDataConsumer::checkSnapshotSync() -> void
	{
		...
		{
			logger_.log("%:% %() % Returning because have not seend SNAPHOT_START yet.\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_)); 
			snapshot_queued_msgs_.clear(); 
			return; 
		}
		...
		{
			logger_.log("%:% %() % Returning because have not seen SNAPSHOT_END yet.\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_)); 
			return; 
		}
		...
		snapshot_mcast_socket_.leave(snapshot_ip_, snapshot_port_); /*; was removed herer*/
	}
	...
	auto MarketDataConsumer::recvCallback(McastSocket *socket) noexcept -> void
	{
		...
		START_MEASURE(Trading_MarketDataConsumer_recvCallback);
	       /*Remember thta "socket_fd_" is in a struct*/	
		...
		if (socket->next_rcv_valid_index_ >= sizeof(Exchange::MDPMarketUpdate))
		{
			...
			for (; i + sizeof(Exchange::MDPMarketUpdate) <= socket->next_rcv_valid_index_; i += sizeof(Exchange::MDPMarketUpdate))
			{
				...
				if (UNLIKELY(in_recovery_))
				{
					...
					{/*if we just entered recovery, start the snapshot synchronization process by subscribing to the snapshot multicast stream.*/
