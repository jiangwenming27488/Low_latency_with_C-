...
namespace Exchange
{
	MarketDataPublisher::MarketDataPublisher(MEMarketUpdateLFQueue *market_updates, const std::string &iface, const std::string &snapshot_ip, int snapshot_port, const std::string &incremental_ip, int incremental_port) : outgoing_md_updates_(market_updates), snapshot_md_updates_(ME_MAX_MARKET_UPDATES), run_(false), logger_("exchange_market_data_publisher.log"), incremental_socket_(logger_)
	{
	...
	auto MarketDataPublisher::run() noexcept -> void
	{
		...
		while (run_)
		{
			/*"market_update" is casted to the "struct outgoing_md_updates_" which is accessing the struct var/function called "getNextToRead()" which is a method to walk to the next node in the link list.
			 * Next, if "outgoing_md_updates_->size()" and "market_update" are true which also means that there is a next in line node.
			 * last, if the loop isnt broken or hit "null" update/increment "market_update" to the next node in the link list with structure*/
