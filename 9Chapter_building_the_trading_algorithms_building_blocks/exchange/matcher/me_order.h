...
namespace Exchange 
{
	struct MEOrder
	{
		...
		/*only need for use with MemPool*/
		MEOrder() = default; 
	...
	struct MEOrdersAtPrice
	{
		...
		atuo to string() const 
		{
			std::stringstream ss; 
			ss << "MEOrdersAtPrice[" << "side:" << sideToString(side_) << " " << "price:" << priceToString(price_) << " " << "first_me_order:" << (first_me_order ? prev_entry_->price_ : Price_INVALID) << " " << "next:" << priceToString(next_entry_ ? next_entry_->price_ : Price_INVALID) << "]"; 
			return ss.str(); 
		}
	}; 
