...
namespace Exchange
{
	...
	class MEOrderBook final 
	{
		public: 
			...
			/*Deleted default, copy and move constructors and assignment-operators*/
			MEOrderBook() = delete; 
		...
		private: 
			...
			auto addOrdersAtPrice(MEOrdersAtPrice *new_orders_at_price) noexcept
			{
				...
				} else
				{
					...
					{/*add new_orders_at_price before target*/
						...
						if ((new_orders_at_price->side_ == Side::BUY && new_orders_at_price->price_ > best_orders_by_price->price_) || (new_orders_at_price->side_ == Side::SELL && new_orders_at_price->price_ < best_orders_by_price_))
						{
			...
			auto addOrder(MEOrder *order) noexcept
			{
				...
				} else 
				{
					...
					order_>next_order_ = first_order; 
