...
namespace Exchange
{
	class OrderServer
	{
			...
			auto recvCallback(TCPSocket *socket, Nanos rx_time) noexcept
			{
				...
				if (socket->next_rcv_valid_index_ >= sizeof(OMClientRequest))
				{
					...
					for (; i + sizeof(OMClientRequest) <= socket->next_rcv_valid_index_; i += sizeof(OMClientRequest))
					{
						...
						{/*Change this to send a reject back to the client, you can just ad a UDP socket option with a SMTP client to email to user email box*/
		...
		private: 
			 ...
			 /*Hash map from ClienId -> the next sequence number to be sent on outgoing client responses.*/
			 ...
			 std::array<Common::TCPSocket *, ME_MAX_CLIENTS> cid_tcp_socket_; 
