...
namespace Exchange
{
	...
	{
		public: 
			FIFOSequencer(ClientRequestLFQueue *client_requests, Logger *logger) : incoming_requests_(client_requests), logger_(logger)
		{
		...
		private: 
			...
			struct RecvTimeClientRequest
			{
				...
				auto operator<(const RecvTimeClientRequest &rhs) const /*If your coming from python like lang and dont really create classes with greater than method RE-WRITE THIS UNTIL ITS SUCK IN YOUR MEMORY(goat level coding method here)*/
				{
					return (recv_time_ < rhs.recv_time_); 
				} /*This is the end of it so simple but SO POWERFULL*/
			}; 
