...
int main(int, char **)
{
	...
	market_data_publisher = new Exchange::MarketDataPublisher(&market_updats, mkt_pub_iface, snap_pub_ip, snap_pub_port, inc_pub_ip, inc_pub_port); 
	market_data_publisher->start(); 
	
	/*also "gw" stands for "gat way" and "ogw" stands for "order gate way"*/
	...
	while (true)
	{
		logger->log("%:% %() % Sleeping for a few milliseconds...\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time-str)); 
