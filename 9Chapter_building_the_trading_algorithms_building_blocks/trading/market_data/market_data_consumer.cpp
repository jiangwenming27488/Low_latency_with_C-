...
namespace Trading
{
	MarketDataConsumer::MarketDataConsumer(Common::ClientId client_id, Exchange::MEMarketUpdateLFQueue *market_updates, const std::string &iface, const std::string &snapshot_ip, int snapshot_port, const std::string &incremental_ip, int incremental_port) : incoming_md_updates_(market_updates), run_(false), logger_("trading_market_data_consumer_" + std::to_string(client_id) + ".log"), incremental_mcast_socket_(logger_), snapshot_mcast_socket_(logger_), iface_(iface), snapshot_ip_(snapshot_ip), snapshot_port_(snapshot_port)
	{
	...
	/*Main loop for this thread - reads and processes messages from the multicast sockets - the heavy lifting is the "recvCallback()" and "checkSnapshotSync()" methods*/
	auto MarketDataConsume::run() noexcept -> void 
	{
		logger_.log("%:% %() %\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTrimeStr(&time_str_)); 
		while (run_)
		{
	...
	auto MarketDataConsumer::checkSnapshotSync() -> void 
	{
		...
		{
			logger_.log("%:% %() % Retruning because have not seen a SNAPSHOT_START yet.\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_)); 
			snapshot_queued_msgs_.clear; 
			return; 
		}
		...
		for (auto &snapshot_itr : snapshot_queued_msgs_)
		{
			...
			if (snapshot_itr.first != next_snapshot_seq)
			{
				...
				logger_.log("%:% %() % Detected gap in snapshot stream expected:% found:% %.\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_), next_snapshot_seq, snapshot_itr.first, snapshot_itr.second.toString()); 
				break; 
			}

			if (snapshot_itr.second.type_ != Exchange::MarketUpdateType::SNAPSHOT_START && snapshot_itr.second.type_ != Exchange::MarketUpdateType::SNAPSHOT_END) final_events.push_back(snapshot_itr.second); 
		...
		{
			logger_.log("%:% %() % Checking nex_exp:% vs. seq:% %\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_), next_exp_inc_seq_num_, inc_itr->first, inc_itr->second.toString()); 
			...
			if (inc_itr->first != next_exp_inc_seq_num_)
			{
				logger_.log("%:% %() % Detected gat in incremental stream expected:% found:% %\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_), next_exp_inc_seq_num_, inc_itr->first, inc_itr->second.toString()); 
		...
		logger_.log("%:% %() % Recovered % snapshot and % incremental orders.\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_), snapshot_queued_msgs_.size() - 2, num_incrementals); 

		snapshot_queued_msgs_.clear(); 
		incremental_queued_msgs_.clear(); 
		in_recovery_ = false; 

		snaphot_mcast_socket_.leave(snapshot_ip_, snapshot_port_);/*';' was added after the first ';' this is not the compiled version yet but learning process so i commit it out because thats wrong syntax on this level*/
	}

	/*Queue up a message in the "*_queued_msgs_" container, first parameter specifies if this update came from the snapshot or the incremental streams*/
	...
	/*Process a market data update, the consumer need to use the socket parameter to figure out whether this came from the snapshot or the incremental stream*/
	auto MarketDataConsumer::recvCallback(McastSocket *socket) noexcept -> void
	{
		...
		if (socket->next_rcv_valid_index_ >= sizeof(Exchange::MDPMarketUpdate))
		{
			...
			for (; i + sizeof(Exchange::MDPMarketUpdate) <= socket->next_rcv_valid_index_; i += sizeof(Exchange::MDPMarketUpdate))
			{
				...
				logger_.log("%:% %() % Received % socket len:% %\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_), (is_snapchat ? "snapshot" : "incremental"), sizeof(Exchange::MDPMarketUpdate), request->toString()); 
				...
				if (UNLIKELY(in_recovery_))
				{
					...
					{/*if we just entered recovery, start the snapshot synchoniztion process by subscribing to the snapshot multicast stream.*/
			...
			memcpy(socket->inboud_data_.data(), socket->inbound_data_.data() + i, socket->next_rcv_valid_index_ - i); 
