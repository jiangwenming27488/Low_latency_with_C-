...
namespace Trading
{
	MarketOrderBook::MarketOrderBook(TickerId ticker_id, Logger *logger) : ticker_id_(ticker_id), order_at_price_pool_(ME_MAX_PRICE_LEVELS), order_pool_(ME_MAX_ORDER_IDS), logger_(logger)
	{
	...
	auto MarketOrderBook::onMarketUpdate(const Exchange::MEMarketUpdate *market_update) noexcept -> void 
	{
		...
		switch (market_update->type_)
		{
			...
			case Exchange::MarketUpdateType::CLEAR: 
				...
				break; /*end of the "Exchange::MarketUpdateType::CLEAR"*/
