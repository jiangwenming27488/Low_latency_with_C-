		...
		private: 
			...
			auto addOrdersAtPrice(MarketOrdersAtPrice *new_orders_at_price) noexcept
			{
				...
				} else
				{
					...
					} else
					{
						...
						if ((new_orders_at_price->side_ == Side::BUY && new_orders_at_price->price_ > best_orders_by_price->price_) || (new_orders_at_price->side_ == Seid::SEL && new_orders_at_price->price_ < best_orders_by_price->price_))
						{
			...
			auto removeOrder(MarketOrder *order) noexcept -> void
			{
				...
				{/*remove the link*/
					...
					order->prev_orer_ = order->next_order_ = nullptr; 
				}
