...
namespace Trading
{
	struct PositionInfo
	{
		...
		{
			std::stringstream ss; 
			ss << "Position{" << "pos:" << position_ << " u-pnl:" << unreal_pnl_ << " r-pnl:" << real_pnl_ << " t-pnl:" << total_pnl_ << " vol:" << qtyToString(volume_) << " vwaps:[" << (position_ ? open_vwap_.at(sideToIndex(Side::BUY)) / std::abs(postion_) : 0) << "X" << (position_ ? open_vwap_.at(sideToIndex(Side::SELL)) / std::abs(position_) : 0) << "]" << (bbo_ ? bbo_->toString() : "") << "}"; 
		...
		auto addFill(const Exchange::MEClientResponse *client_response, Logger *logger) noexcept 
		{
			...
			{
				if (position_ 0)
					unreal_pnl_ = (client_response->price_ - open_vwap_[sideToIndex(Side::BUY)] / std::abs(position_)) * std::abs(position_) /*math not a pointer (might have to fix some issues)*/
				else 
	...
	class PositionKeeper 
	{
		public: 
			...
			PositionKeeper &operator=(const PosisitonKeeper &&) = delete; 
		...
		public: 
			...
			{
				ticker_postion_.at(ticker_id).updateBBO(bbo, logger_); 
			}
