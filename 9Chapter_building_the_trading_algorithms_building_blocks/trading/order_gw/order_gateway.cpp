...
namespace Trading
{
	...
	auto OrderGateway::recvCallback(TCPSocket *socket, Nanos rx_time) noexcept -> void
	{
		...
		{
			size_t i = 0; 
			for (; i + sizeof(Exchange::OMClinetResponse) <= socket->next_rcv_valid_index_; i += sizeof(Exchange::OMClientResponse))
			{
				...
				{/*This should never happen unless there is a bug in the exchange*/
				...
				{/*This should never happen since we use a TCP protocol, unless there is a bug at the exchange*/
