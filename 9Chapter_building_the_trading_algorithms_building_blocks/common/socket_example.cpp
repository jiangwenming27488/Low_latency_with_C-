...
int main(int, char **)
{
	...
	auto tcpServerRecvCallback = [&](TCPSocket *soket, Nanos rx_time) noexcept
	{
		logger_.log("TCPServer::defaultRecvCallback() socket:% len:% rx:%\n", socket->socket_fd_, socket->next_rcv_valid_index_, rx_time); 

		const std::string reply = "TCPServer receive msg:" + std::string(socket->inbound_data_.data(), socket->next_rcv_valid_index_); 
	...
	{
		clients[i] = new TCPServer(logger_); 
	...
	for (auto itr = 0; itr < 5; ++itr)
	{
		for (size_t i = 0; i < clients.size(); ++i)
		{
			...
			logger_.log("Sending TCPClinet-[%] %\n", i, client_msg); 
			clients[i]->send(client_msg.data(), client_msg.length()); 
			cleints[i]->sendAndRecv(); 
