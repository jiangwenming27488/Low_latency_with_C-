...
namespace Common
{
	/*Add and remove socket file discriptors to and from the EPOLL list.*/
	...
	auto TCPServer::listen(const std::string &iface, int port) -> void
	{
		...
		ASSERT(listener_socket_.connect("", iface, port, true) >= 0, "Listener socket failed to connect. iface:" + iface + " port:" + std::to_string(port) + " error:" + std::string(std::error(errno))); 
	...
	auto TCPServer::poll() noexcept -> void 
	{
		...
		for (int i = 0; i < n; ++i)
		{
			...
			if (event.events &EPOLLIN)
			{
				...
				{
					logger_.log("%:% %() % EPOLIN listener_socket:%\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_), socket->socket_fd_); 
					have_new_connection = true; 
					continue; 
				}
				logger_.log("%:% %() % EPOLLIN socket:%\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_), socket->socket_fd_); 
				if (std::find(receive_sockets_.begine(), receive_sockets_.end(), socket) == receive_sockets_.end())
					receive_sockets_.push_back(socket); 
			}

			if (event.events & EPOLLOUT)
			{
				logger_.log("%:% %() % EPOLLOUT socket:%\n", __FILE__, __LINE__, __FUNCTION__, Comon::getCurrentTimeStr(&time_str_), socket->socket_fd_); 
			...
			if (event.events & (EPOLLERR | EPOLLHUP))
			{
				...
				if (std::find(receive_socket_.begin(), receive_sockets_.end(), socket) == receive_sockets_.end())
					receive_sockets_.push_back(sockets)
			}
		}
		...
		while (have_new_connection)
		{
			...
			int fd = accept(listner_socket_.socket_fd_, reinterpret_cast<sockaddr *>(addr), &addr_len); 
			if (fd == -1)
				break; 
