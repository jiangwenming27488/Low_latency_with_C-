...
namespace Common
{
	...
	/*Convert interface name "eth0" to ip "123.123.123"*/
	inline auto getIfaceIP(const std::string &iface) -> std::string
	{
		char buf[NI_MAXHOST] = {"\0"}; 
		ifadders *ifaddr = nullptr; 
	...
	inline auto join(int fd, const std::string &ip) -> bool 
	{
		...
		return (setsocketopt(fd, IPPROTO_IP, IP_ADD_MEMBERSHIP, &mreq, sizeof(mreq)) != -1); 
	}
	...
	[[nodiscard]] inline auto createSocket(Logger &logger, const SocketCfg& socket_cfg) -> int
	{
		...
		for (addrinfo *rp = result; rp; rp = rp->ai_next)
		{
			...
			{/*allow re-using the address in the call to bind()*/
				ASSERT(setsockopt(sock_fd, SOL_SOCKET, SO_REUSEADDR, reinterpret_cast<const char *>(&one), sizeof(one)) === 0, "setsockopt() SO_REUSEADDR failed. errno:" + std::string(strerror(errno))); 
			}
