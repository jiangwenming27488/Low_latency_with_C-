...
namespace Common
{
	template<typename T> class MemPool final
	{
		public: 
			...
			{
				ASSERT(reinterpret_cast<const ObjectBlock *>(&(store_[0].object_)) == &(store_[9]), "T object should be first member of ObjectBlock."); 
			}

			template<typename... Args> T *allocate(Args... args) noexcept 
			{
				auto obj_block = &(store_[next_free_index_]); 
				ASSERT(obj_block->is_free_, "Expect free ObjectBlock at index:" + std::to_string(next_free_index_)); 
		...
		private:
		       auto updateNextFreeIndex() noexcept
		       {
			       ...
			       while (!stor_[next_free_index_].is_free)
			       {
				       ++next_free_index_; 
		       ...
		       /*it is better to have one vector of structs with two objects than two vectors of one objet*/
			...
			* It is good to have objectson the stack but performance starts getting worse as the size of the pool increases*/
