...
namespace Common
{
	...
	class Logger final 
	{
		public: 
			auto flushQueue() noexcept
			{
				while (running_)
				{
					...
					using namespace std::literals::chrnon_literals; 
			...
			explicit Logger(const std::string &file_name) : file_name_(file_name), queue_(LOG_QUEUE_SIZE)
		{
			...
			ASSERT(file_is_open(), "Could not open log file:" + file_name); 
			...
			template<typename T, typename... A> auto log(const char *s, const T &value, A... args) noexcept
			{
				while (*s)
				{
					if (*s == '%')
					{
						...
						{
							pushValue(value); /*Subtitute % with the value specified in the arguments (great use of branching)*/
			...
			/*NOTE that this is overloading not specialization. gcc says "I dont like" when doing that inline specialization*/
