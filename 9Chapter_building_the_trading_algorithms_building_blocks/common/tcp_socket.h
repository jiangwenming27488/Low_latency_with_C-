...
namespace Common
{
	...
	struct TCPSocket
	{
		...
		{
			outbound_data_.resize(TCPBuffersSize); 
			inbound_data_.resize(TCPBufferSize); 
		}

		/*Create TCPSocket with provided atrributes to either listen-on / connect-to*/
		...
		/*Deleted default, copy & move constructors and assignment_operators*/
		TCPSocket() = delete; 
