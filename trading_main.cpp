...
int main(int argc, char **argv)
{
	if(argc < 3)
	{
		FATAL("USAGE trading_main CLIENT_ID ALGO_TYPE [CLIP_1 THRESH_1 MAX_ORDER_SIZE_1 MAX_LOSS_1] [CLIP_2 THRESH_2 MAX_ORDER_SIZE_2 MAX_POS_2 MAX_LOSS_2] ..."); 
	}
	...
	/*The lock free queues to facilitate communcation between order gateway <-> trade engine and market data consumer -> trade engine.*/
	...
	{
		logger->log("%:% %() % Waiting till no activily, been silent for % seconds...\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str), trade_engine->silentSeconds()); 
