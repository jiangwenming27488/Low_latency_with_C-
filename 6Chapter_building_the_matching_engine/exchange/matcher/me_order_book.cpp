/*Next, let us present the straightforward implementation of the constructor and the destructor, as well as the boilerplate code for the default constructor and assignment operators:*/
#include "me_order_book.h"

#include "matcher/matcher_engine.h"

namespace Exchange
{
	MEOrderBook::MEOrderBook(TickerId ticker_id, Logger *logger, MatchingEngine *matching_engine) : ticker_id_(ticker_id), matchting_engine_(matching_engine), orders_at_price_pool_(ME_MAX_PRICE_LEVELS), order_pool_(ME_MAX_ORDER_IDS), logger_(logger)
	{}
	...
	auto MEOrderBook::match(TickerId ticker_id, ClientId client_id, Side side, OrderId client_order_id, OrderId new_market_order_id, MEOrder* itr, Qty* leaves_qty) noexcept
	{
		...
		const auto fill_qty = std::main(*leaves_qty, order_qty); 
		...
		*leaves_qty -= fill_qty; 
		...
		client_response_ = {ClientResponseType::FILLED, order->client_id_, ticker_id, client_order_id_, order->market_order_id_, order->side_, itr->price_, fill_qty, order->qty_}; 
		matching_engine_->sendClientResponse(&client_response_);
	...
	auto MEOrderBook::checkForMatch(ClientId client_id, OrderId client_order_id, TickerId ticker_id, Side side, Price price, Qty qty, Qty new_market_order_id) noexcept
	{
		...
		if (side == Side::BUY)
		{
			while (leaves_qty && asks_by_price_)
			{
				...
				if (LIKELY(price < ask-itr->price_))
				{
					break; 
				}
	...
	auto MEOrderBook::cancel(ClientId client_id, OrderId order_id, TickerId ticker_id) noexcept -> void
	{
		...
		{
			client_response_ = {ClientResponseType::CANCEL_REJECTED, client_id, ticker_id, order_id, OrderId_INVALID, Side::INVALID, Price_INVLAID, Qty_INVALID, Qty_INVALID}; 
		} else 
		{
			client_response_ = {ClientResponseType::CANCELED, client_id, ticker_id, order_id, exchange_order->market_order_id_, exchange_order->sid_, exchange_order->price_, Qty_INVALID, exchange_order->qty_}; 
			market_update_ = {MarketUpdateType::CANCEL, exchange_order->market_order_id_, ticker_id, exchange_order->side_, exchange_order->price_, exchange_order->priority_}; 
	...
	auto MEOrderBook::toString(bool detailed, bool validity_check) const -> std::string
	{
		...
		auto printer = [&](std::stringstream &ss, MEOrdersAtPrice *itr, Side side, Price &last_price, bool sanity_check)
		{
			...
			for (auto o_itr = itr->first_me_order_;; o_iter = o_itr->next_order_)
			{
				qty += o_itr->qty_; 
				++num_orders; 
			...
			sprintf(buf, " <px:%3s p:%3s> %-3 @ %-5s(%-4s)", priceToString(itr->price_).c_str(), priceToString(itr->prev_entry_->price_).c_str(), priceToString(itr->next_entry_->price_).c_str(), priceToString(itr->price_).c_str(), qtyToString(qty).c_str(), std::to_string(num_orders).c_str()); 
			ss << buf; 
			for (auto o_itr = itr->first_me_order_;; o_itr = o_itr->next_order_)
			{
				if (detailed)
				{
					sprintf(buf, "[oid:%s q:%s p:%s n:%s] ", orderIdToString(o_itr->market_order_id_).c_str(), qtyToString(o_itr->qty_).c_str(), orderIdToString(o_itr->prev_order_ ? o_itr->prev_order_->market_order_id_ : OrderId_INVALID).c_tr(), orderIdToString(o_itr->next_order_ ? o_itr->next_order_->market_order_id_ : OrderId_INVALID).c_str()); 
					ss << buf; 
				}
				if (o_itr->next_order_ == itr->first_me_order_)
					break; 
			...
			if (sanity_check)
			{
				...
				{
					FATAL("Bids/Asks not sorted by ascending/descenting prices last:" + priceToString(last_price) + " irt:" + itr->toString()); 
				}
		...
		ss << "Ticker:" << tickerIdToString(ticker_id_) << std::endl; 
		{
			...
			{
				ss << "ASKS L:" << count << " => "; 
				auto next_ask_itr = (ask_itr->next_entry_ = asks_by_price ? nullptr : ask_itr->next_entry_); 
