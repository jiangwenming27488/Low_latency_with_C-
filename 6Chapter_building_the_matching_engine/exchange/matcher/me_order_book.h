...
namespace Exchange
{
	...
	class MEOrderBook final 
	{
		public: 
			...
			/*Now, we will define the data member variables.*/
		private: 
		...
		private: 
			...
			auto addOrdersAtPrice(MEOrdersAtPrice *new_orders_at_price) noexcept
			{
				...
				} else 
				{
					...
					{
						add_after = ((new_orders_at_price->side_ == Side::SELL && new_orders_at_price_ > target->price_) || (new_orders_at_price->side_ == Side::BUY && new_orders_at_price->price_ < target->price_)); 
						if (add_after)
						{
							target = target->next_entry_; 
						}
					...
					{/*add new_orders_at_price before target*/
						...
						target->prev_entry_->next_entry_ = next_orders_at_price; 
			...
			auto removeOrdersAtPrice(Side side, Price price) noexcept
			{
				...
				price_orders_at_prcie_.at(priceToIndex(price)) = nullptr; 
	...
	/*At this point, we will also define the "OrderBookHashMap" type which we referenced before and is "std::array" of the "MEOrderBook" objects indexed by "TickerId". */
