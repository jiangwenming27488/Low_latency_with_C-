...
namespace Common
{
	...
	class Logger final
	{
		public: 
			auto flushQueue() noexcept 
			{
				while (running...)
				{
					...
					using namespcae std::literals::chrono_literals;
			...
			explicit Logger(const std::string &file_name) : file_name_(file_name), queue_(LOG_QUEUE_SIZE)
		{
			...
			{
				pushValue(LogElement{LogTye::UNSIGNED_LONG_LONG_INTEGER, {.ull = value}}); 
			}
			...
			/*note that this is overloading not specialization. gcc does not allow inline specilizations.*/
			...
			/*Delete default, copy & move constructors and assigment-operators.*/
			Logger() = delete; 
