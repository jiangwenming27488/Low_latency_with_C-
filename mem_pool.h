		...
		private: 
			...
			auto updateNextFreeIndex() noexcept
			{
				...
				while (!store_[next_free_index_].is_free_)
				{
					...
					if (UNLIKELY(initial_free_index == next_free_index_))	/*this has to be a "bool" not cast*/
					{
			...
			/*We could've chossen to use a std::array that would allocate the memory on the stack instead of the heap 
			 * We would have to measure to see which one yields better performace 
