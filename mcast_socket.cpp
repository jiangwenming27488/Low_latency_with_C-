...
namespace Common
{
	/*Initialize multicast socket ot read from or publish to a stream
	 * Does not join the muticast stream yet*/
	...
	auto McastSocket::send(const void *data, size_t len) noexcept -> void
	{
		...
		ASSERT(next_send_valid_index_ < McastBufferSize, "Mcast socket buffer filled up and sendAndRecv() no called."); 
	}
}
