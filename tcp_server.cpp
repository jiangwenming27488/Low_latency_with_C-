...
namespace Common
{
	...
	/*Publish outgoin data from the send buffer and read incoming data from the receive buffer.*/
	...
	/*Check for new connections or dead connections and update container that track the sockets*/
	auto TCPServer::poll() noexcept -> void
	{
		...
		/*Accept a new connection, create a "TCPSocket" and add it to out containers.*/
		while (have_new_connection)
		{
			logger_.log("%:% %() % have_connection\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_)); 
