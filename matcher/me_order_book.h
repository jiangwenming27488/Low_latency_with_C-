...
namespace Exchange
{
	...
	class MEOrderBook final 
	{
		public: 
			 ...
			 * It will check to see if this new order matches and existing passive order with opposite side, and perform the matching if that is the case.*/
			auto add(ClientId client_id, OrderId client_order_id, TickerId ticker_id, Side side, Price price, Qty qty) noexcept -> void; 

			/*Attempt to cancel and order in the order book, issue a cancel-rejection if order does not exist*/
		...
		private: 
			...
			/*The parent matching engine instance, usded to publish market data and client responses*/
		...
		private: 
			...
			auto addOrdersAtPrice(MEOrdersAtPrice *new_orders_at_price) noexcept
			{
				...
				} else 
				{
					...
					bool add_after = ((new_orders_at_price->side_ == Side::SELL && new_orders_at_price->price_ > target->price_) || (new_orders_at_price->side_ == Side::BUY && new_orders_at_price->price_ < target->price_)); 
					if (add_after)
					{
					...
					{/*add "new_orders_at_price" before target*/
						...
						/*"new_orders_at_price->side_" is equlavent to "Side::BUY" and "new_orders_at_price->price_" which if true should also be larger then "best_orders_by_price->price_" and is greater then "best_orders_at_price->price_"
						 * OR "new_orders_at_price->side_" is equlavent to "Side::SELL" and "new_orders_at_price->price_" is smaller then "best_orders_by_price->price_" 
						 * if one condition works do the processing*/
			...
			/*Check if a new order with the provided attributes would match against existing passive order held in the "bid_itr" object and generate client responses and marketupdate for the match
			 * It will update the passive order " (bid_itr)" based on the match and possibly remove it if fully matched. 
			 * It will return remaing quantity on the aggressive order in the "leaves_qty" parameter*/
			...
			/*Check if a new order with the provided attributes would match against existing passive orders on the orther side of the order book. 
