...
namespace Common
{
	/*Constants usded across the ecosystem to represent uppder bounds on various containters
	 * Trading instruments / TikcersIds from [0, ME_MAX_TICKERS]*/
	constexpr size_t ME_MAX_TICKERS = 8; 

	/*Maximum size of lock free queues used to transfer client requests, client response and market updates between components */
	...
	/*Maximum number o forders per trading client*/
	...
	/*Proirity represents position in the FIFO queue for all orders with the same side and price attributes*/
	...
	struct RiskCfg
	{
		...
		auto toString() const 
		{
			...
			ss << "RiskCfg{" << "max-order-size:" << qtyToString(max_order_size_) << " " << "max_position:" << qtyToString(max_position_) << " " << "max-loss:" << max_loss_ << "}"; 
	...
	struct TradeEngineCfg
	{
		...
		{
			std::stringstream ss; 
			ss << "TradeEngineCfg{" << "clip:" << qtyToString(clip_) << " " << "thresh:" << threshold_ << " " << "risk:" << risk_cfg_.toString() << "}"; 
